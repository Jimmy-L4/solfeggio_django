...
class SightsingingList(APIView):
    ...
    def get(request):
        ...
        stateText = ['已完成', '待完成', '已逾期']
        stateType = ['check-circle', 'clock-circle', 'stop']
...
class ChoiceList(APIView):
    ...
    def get(request):
        ...
        stateText = ['已完成', '待完成', '已逾期']
        stateType = ['check-circle', 'clock-circle', 'stop']
        stateDec = ['题目已经完成！点击作答查看题目详情，作业无法重复提交。',
                    '题目待完成，快点击作答完成作业吧！',
        ...
        for index, ques in enumerate(data):
            state = random.randint(0, 2)
            quesInfo = {'id': ques['id'], 'part_id': ques['part_id'], 'stateText': stateText[state],
